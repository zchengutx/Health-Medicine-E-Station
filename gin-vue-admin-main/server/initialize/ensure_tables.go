...
// 确保审核记录表存在
func ensureApprovalTables() {
	// 创建医生审核记录表
	if !global.GVA_DB.Migrator().HasTable(&medicine.MtDoctorApproval{}) {
		err := global.GVA_DB.AutoMigrate(&medicine.MtDoctorApproval{})
		if err != nil {
			global.GVA_LOG.Error("创建医生审核记录表失败", zap.Error(err))
		} else {
			global.GVA_LOG.Info("医生审核记录表创建成功")
		}
	}
}
