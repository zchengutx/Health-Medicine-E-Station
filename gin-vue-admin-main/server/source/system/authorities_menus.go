...
func (i *initMenuAuthority) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	// 2. 普通用户角色(8881) - 拥有基础功能菜单和药品管理菜单
	// 仅选择部分父级菜单及其子菜单
	var menu8881 []sysModel.SysBaseMenu

	// 添加仪表盘、关于我们、个人信息和药品管理菜单
	for _, menu := range allMenus {
		if menu.ParentId == 0 && (menu.Name == "dashboard" || menu.Name == "about" || menu.Name == "person" || menu.Name == "state" || menu.Name == "medicine") {
	...
	for _, menu := range allMenus {
		...
		if menu.ParentId > 0 && parentName == "medicine" {
			menu8881 = append(menu8881, menu)
		}
	}
